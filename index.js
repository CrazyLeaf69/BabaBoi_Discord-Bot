...
const ForbiddenChannels = ["689545134957789212", "824303080211611730", "882313340876107777", "886324456966995988"];
...
client.on("ready", async () => {
  ...
  // client.user.setPresence({ activities: [{ name: 'Made by Luke9595' }] });
  client.user.setActivity("HARDSTYLE", { type: "LISTENING" });
...
async function play(search, interaction) {
  await fetch(
    `https://youtube.googleapis.com/youtube/v3/search?part=snippet&q=${search}&type=video&key=AIzaSyD4q3HFuGrKvo7qpB0-wsJYWnKiWwZGILM`
  )
    .then((res) => res.json())
    .then(async (data) => {
...
async function playQueue(interaction) {
  ...
  if (voiceChannel) {
    ...
    const dispatcher = connection.play(ytdl(queue[0].url, { filter: "audioonly" }));
...
client.on("messageCreate", async (message) => {
  ...
  //     message.channel.send("fortnite is gay")
  ...
  //         player.play(createAudioResource(ytdl("https://www.youtube.com/watch?v=bwD8G3a86p8")));
  ...
  if (message.content.substring(0, 1) == prefix && message.guild.id != "651518721327628308") {
    // ForbiddenChannels.forEach(element => {
    //     if (message.channel.id == element) {
    //         message.reply("forbidden channel")
    //         return;
    //     }
    // });
    ...
    if (timestamps.has(message.author.id)) {
      ...
      if (now < expirationTime) {
        ...
        return message.reply(
          `please wait ${timeLeft.toFixed(1)} more seconds before reusing the \`${command.name}\` command.\nCooldown: ${
            command.cooldown
          } Seconds`
        );
      }
